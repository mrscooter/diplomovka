...
Uvažujme množinu dvojíc slov $ F_n = \lbrace (a^{l},ba^{n-l}), (a^{l}b,a^{n-l}) \; | \; 0 \leq l \leq n-1 \rbrace $. Podla definície \ref{def:extended_fooling_set} je množina $ F_n $ rozšírenou oblbovacou množinou. $ |F_n|=2n $, teda podla Vety \ref{thm:extended_fooling_set_technique} $ nsc(L_n) \geq 2n$. Kedže $ L(A_n) = L_n $ a $ \#_S(A_n) = n+2 $, tak $nsc(L_n) = 2n $ a automat $ A_n $ je minimálny NKA pre jazyk $ L_n $.
\par

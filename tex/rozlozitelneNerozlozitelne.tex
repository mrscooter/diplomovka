...
Uvažujme množinu dvojíc slov $ F_n = \lbrace (a^{l},ba^{n-l}), (a^{l}b,a^{n-l}) \; | \; 0 \leq l \leq n-1 \rbrace $. Podla definície \ref{def:extended_fooling_set} je množina $ F_n $ rozšírenou oblbovacou množinou pre jazyk $ L_n $. $ |F_n|=2n $, teda podla Vety \ref{thm:extended_fooling_set_technique} $ nsc(L_n) \geq 2n$. Kedže $ L(A_n) = L_n $ a $ \#_S(A_n) = n+2 $, tak $nsc(L_n) = 2n $ a automat $ A_n $ je minimálny NKA pre jazyk $ L_n $.
\par
...
Teraz uvažujmne množinu dvojíc slov $ F = \lbrace (a^i,a^{l-i}b^k),(a^lb^j,b^{k-j}) | 0 \leq i \leq l, \; 1 \leq j \leq k \rbrace $. Množina $ F $ je podľa definície \ref{def:fooling_set} oblbovacou množinou pre jazyk $ L_{l,k} $. Kedže $ |F| = l+k+1 $, tak podľa Vety \ref{thm:fooling_set_technique} platí $ nsc(L_{l,k}) \geq l+k+1 $. Nakoľko $ L(A_{l,k})=L_{l,k} $ a $ \#_S(A_{l,k})=l+k+1 $, tak $ nsc(L_{l,k}) = l+k+1 $ a automat $ A_{l,k} $ je minimálnym NKA pre jazyk $ L_{l,k} $.
\par
